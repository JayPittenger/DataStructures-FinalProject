...
class HashMap:
    ...
    def clear(self):
        ...
        """
        # remake empty list and populate with empty linkedLists
        self._buckets = []
    ...
    def get(self, key):
        ...
        """
        # determine bucket where key should be located
    ...
    def resize_table(self, capacity):
        ...
        """
        # make new array of requested capacity
        new_array = []
        for i in range(capacity):
            new_array.append(LinkedList())
        self.capacity = capacity
        # perform rehash of nodes based on new capacity and add to new_array
        for bucket in self._buckets:
            if bucket.head is not None:
                ...
                while cur is not None:
                    ...
                    index = hash % capacity
                    new_array[index].add_front(cur.key, cur.value)
                    cur = cur.next
        # set Hash Map array to new array
    ...
    def put(self, key, value):
        ...
        # check the bucket for the key
        node = bucket.contains(key)
        # update value if already there
        if node is not None:
            node.value = value
            return
        # add value if not yet there
        else:
    ...
    def empty_buckets(self):
        ...
        for buckets in self._buckets:
            # if linkedList is empty, count bucket as empty
    ...
    def table_load(self):
        ...
        for buckets in self._buckets:
            bucket_count += 1
            # number of links is equal to the length of linkedList - add to count of all links
            links += buckets.size
        return float(links / bucket_count)
